...
* At first we need to build our training set, which can be shown as $$\omega$$, $$\omega = {(**x**,c_i) | i=1,2 \cdots n}$$, where **x** is a vector in dimension *l*, $$x_i^h$$ shows the value h in sample i, $$c_i$$ means the category that sample i belongs to.     
